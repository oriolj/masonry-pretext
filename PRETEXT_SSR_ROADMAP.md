...
| 6 | `pretextOptions` shorthand (the convenience layer) | ✅ `v5.0.0-dev.35` | [035-pretext-options-shorthand.md](./improvements/035-pretext-options-shorthand.md) | new `pretextOptions: { measure, font, text?, padding? }` shorthand that wraps the `pretextify` callback with a built-in WeakMap cache. `computeAndRender` was considered and skipped — Astro/Next.js framework templates handle rendering cleanly without a built-in helper. |
