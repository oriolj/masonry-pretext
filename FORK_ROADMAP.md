...
| **D.2** | **`static: 'until-resize'` hybrid mode** — adopt server positions on first paint AND still relayout on actual viewport changes | downstream consumer ask | ✅ `v5.0.0-dev.45` | [045-static-until-resize.md](./improvements/045-static-until-resize.md) | extends `static` option to accept the string `'until-resize'`; on first window-resize-driven layout, restores `transitionDuration` + wires up the per-item ResizeObserver retroactively. +114 B gz; new `static-until-resize` discriminating fixture; new `_buildResizeObserver` helper extraction. **Tier 1 (D.1, D.2, D.3, D.4) is COMPLETE** |
...
| **D.8** | **`masonry-pretext/astro` integration subpath** — drop-in `<MasonryGrid>` Astro component with the dynamic-import + View Transitions boilerplate built in | downstream consumer ask | ✅ `v5.0.0-dev.49` | [049-astro-integration-subpath.md](./improvements/049-astro-integration-subpath.md) | new `./astro` package export shipping `dist/masonry-astro.{js,mjs}`; side-effect bundle that loads `<masonry-grid>` (#034) + wires `astro:page-load`/`astro:after-swap` listeners that detect stale persisted elements via cheap O(1) heuristic and reconstruct. **Zero bytes added to imperative-API bundles.** New module-smoke case for the astro bundle. **All 12 D.* items now shipped — Tier 1 + Tier 2 + Tier 3 complete** |
| **D.9** | **`replaceItems(newItems)`** — swap the item set without destroy/reconstruct | downstream consumer ask | ✅ `v5.0.0-dev.46` | [046-replace-items.md](./improvements/046-replace-items.md) | thin orchestration of existing API (`baseRemove` + `_itemize` + `concat` + `layout`); +59 B gz; preserves observer state across the swap; new `replace-items` discriminating fixture |
| **D.10** | **`pause()` / `resume()`** for View Transitions | downstream consumer ask | ✅ `v5.0.0-dev.47` | [047-pause-resume.md](./improvements/047-pause-resume.md) | adds `_paused` flag check at entry + inside rAF callback for both ResizeObserver and MutationObserver; resume triggers a catch-up layout; +51 B gz; new `pause-resume` discriminating fixture (verifies layout count delta across the pause boundary) |
| **D.11** | **`Masonry.diagnose()`** structured state snapshot | downstream consumer ask | ✅ `v5.0.0-dev.48` | [048-diagnose.md](./improvements/048-diagnose.md) | typed `MasonryDiagnostic` shape with cols / columnWidth / containerSize / items / observers / lastLayoutTimestamp / lastRelayoutReason; +319 B gz (above predicted band — cost dominated by human-readable status strings); new `diagnose` discriminating fixture exercising every field via pageAssert |
