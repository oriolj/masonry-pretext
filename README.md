...
| `v5.0.0-dev.48` | **`msnry.diagnose()` structured snapshot.** New instance method returning a typed `MasonryDiagnostic` object describing the current state of the grid — cols, columnWidth, container size, items with their positions/sizes/observer-wired flag, observer status strings, last layout timestamp, last relayout reason. Standardized shape for dev tools / testing frameworks instead of every consumer rewriting their own debug helper. Cost: **+319 B gzipped** (above the predicted band — dominated by human-readable status strings; revisit-able in a simplify pass). Closes [downstream consumer ask D.11](./FORK_ROADMAP.md#d11--masonrydiagnose-structured-state-snapshot). Full record in [`improvements/048-diagnose.md`](./improvements/048-diagnose.md). |
| `v5.0.0-dev.49` | **`masonry-pretext/astro` integration subpath.** New `import 'masonry-pretext/astro'` side-effect bundle that loads the `<masonry-grid>` Custom Element + wires up `astro:page-load` / `astro:after-swap` listeners. Detects stale persisted `<masonry-grid>` elements (where Astro swapped contents in-place but `connectedCallback` didn't fire) via an O(1) item-identity heuristic and reconstructs them. **Zero bytes added to imperative-API bundles** — ships as two new separate files (`dist/masonry-astro.{js,mjs}`) that opt-in consumers pay for. Closes [downstream consumer ask D.8](./FORK_ROADMAP.md#d8--masonry-pretextastro-integration-subpath) — **all 12 downstream consumer asks (D.1–D.12) are now landed**. Full record in [`improvements/049-astro-integration-subpath.md`](./improvements/049-astro-integration-subpath.md). |
